...
{
    // Verifica se o saldo regular é suficiente para cobrir a aposta
    if ($wallet->balance >= $bet) {
        $wallet->decrement('balance', $bet);
        return 'balance';
    } 
    // Se o saldo regular não for suficiente, tenta usar o saldo de saque
    elseif ($wallet->balance_withdrawal >= $bet) {
        $wallet->decrement('balance_withdrawal', $bet);
        return 'balance_withdrawal';
    } 
    // Se nem o saldo regular nem o saldo de saque forem suficientes, verifica se o saldo total é suficiente
    elseif (($wallet->balance + $wallet->balance_withdrawal + $wallet->balance_bonus) >= $bet) {
        // Calcula o saldo total disponível para cobrir a aposta
        $totalBalance = $wallet->balance + $wallet->balance_withdrawal + $wallet->balance_bonus;
        // Deduz o valor da aposta do saldo regular
        $betFromBalance = min($wallet->balance, $bet);
        $wallet->decrement('balance', $betFromBalance);
        // Atualiza o valor da aposta a ser deduzido
        $remainingBet = $bet - $betFromBalance;
        // Se ainda houver saldo necessário para cobrir a aposta, deduz do saldo de saque
        if ($remainingBet > 0) {
            $betFromWithdrawal = min($wallet->balance_withdrawal, $remainingBet);
            $wallet->decrement('balance_withdrawal', $betFromWithdrawal);
            // Atualiza o valor restante da aposta
            $remainingBet = $bet - $betFromBalance - $betFromWithdrawal;
        }
        // Se ainda houver saldo necessário para cobrir a aposta, deduz do saldo de bônus
        if ($remainingBet > 0) {
    ...
    // Retorna 'no_balance' se nenhum dos saldos for suficiente para cobrir a aposta
    return 'no_balance';
}
//=======45///////=======45///////=======45///////=======45///////=======45///////=======45/////
//=======45///////=======45///////=======45///////=======45///////=======45///////=======45/////
